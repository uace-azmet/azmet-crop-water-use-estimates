...
# @return: azmetData - transformed data table
...
fxnAZMetDataELT <- function(station, timeStep, startDate, endDate) {
  ...
  if (timeStep == "Hourly") {
    ...
    if (nrow(azmetData) == 0) {
      azmetData <- data.frame(matrix(nrow = 0, ncol = length(c(varsID, varsMeasure))))
      colnames(azmetData) <- c(varsID, varsMeasure)
    } else {
      # Tidy data
      azmetData <- azmetData %>%
